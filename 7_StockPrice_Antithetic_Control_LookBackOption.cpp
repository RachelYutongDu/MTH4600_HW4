...
int main() {
   ...
   double r, t, T, mu, sigma, dt, sqrtdt, S, S0, Z, Zbar, Z2bar,
          ...
          U, B, B_telda, S_telda, K, C, C_telda, BS_call, diff, Smax, L, L_telda, Smax_telda, a, A, Abar, A2bar, ZAbar, Zstar, Zstarbar, Zstar2bar, stdhatZstar;
   int i, N, done, test;
   ...
   //Print title to indicate the purpose of the program
   printf("    Value the look-back option\n");
   ...
   while (!done) {
      ...
      for (i = 1; i <= N; i++) {
         ...
         S_telda = S0 * exp (mu*t + sigma*B_telda);
          ...
          // Compare the value with prior stock price and keep the higher one
         if (Smax < S) {
             Smax = S;
          }
        
         if (Smax_telda < S_telda) {
             Smax_telda = S_telda;
          }
      ...
      L = fmax(Smax - K,0);
      L_telda = fmax(Smax_telda - K,0);
       
      //Compute the Z anthithestic statistic
       Z = (L + L_telda) * 0.5 * Discount_factor;
       
      // #6. implement the control variable on L
      ...
      if (test == 100000) {
         ...
         // Report.
         printf ("%10.0f   %8.4f   %8.4f   %8.3f %8.3f\n", n, Zstarbar, error, elapsed_time, t_star);
